...
public class LunchMenu
{
    ...
    public void LunchList()
    {
        ...
        Lunch_txt_Scanner();
        // Prompts the user to select a Lunch item
        Lunch_Menu_Selector();
    }
    ...
    int [] Lunch_Item_Index = new int[5];
    String [] Lunch_Item_Name = new String[5];
    float [] Lunch_Item_Price = new float[5];
    String [] Lunch_Item_Description = new String[5];
    String [] Lunch_Item_INFO_Combined= new String[5];
    ...
    private void Lunch_txt_Scanner()
    {
        String SectionReader = "";
        try
        {
            //Scanner input = new Scanner(new File("."+System.getProperty("path.separator")+"Lunch.txt"));
            Scanner input = new Scanner(new File("Lunch_Menu.txt"));
            input.useDelimiter(";");
            ...
            for (int Line = 0; Line < 5 ; Line++)
            {
                for(int Section = 0; Section < 4; Section++)
                {
                    if (Section == 0)
                    {
                        ...
                        Lunch_Item_Index[Line] = Integer.parseInt(SectionReader);
                    }
                    else if (Section == 1)
                    {
                        SectionReader = input.next();
                        //System.out.println("This is Name: "  + SectionReader);
                        Lunch_Item_Name[Line] = SectionReader;
                    }
                    else if (Section == 2)
                    {
                        SectionReader = input.next();
                        //System.out.println("This is Price : "  + SectionReader);
                        Lunch_Item_Price[Line] = Float.parseFloat(SectionReader);
                    }
                    else if (Section == 3)
                    {
                        SectionReader = input.next();
                        //System.out.println("This is Description : "  + SectionReader);
                        Lunch_Item_Description[Line] = SectionReader;
                    }
                    Lunch_Item_INFO_Combined[Line] = Items_INFO_Combined_Format(Lunch_Item_Index[Line], Lunch_Item_Name[Line], Lunch_Item_Price[Line], Lunch_Item_Description[Line]);
                }
                input.nextLine();
            }
        }
    ...
    {
        for (int i = 0; i < Lunch_Item_INFO_Combined.length; i++)
        {
            System.out.println(Lunch_Item_INFO_Combined[i]);
        }
    ...
    public static float Lunch_TotalPrice = 0.0f;
    public static int Lunch_TotalQuantity = 0;
    String[] Lunch_Recipt_ItemsList = new String[10];
    ...
    private void Send_Items_To_Recipt()
    {
       ...
       {
        initalCounter = 0;
           while(Lunch_Recipt_ItemsList[initalCounter] != null)
           {
               receipt.ReceiptTotalItems[initalCounter] = Lunch_Recipt_ItemsList[initalCounter];
               initalCounter++;
           }
       }
       ...
       else
       {
            ...
            while(Lunch_Recipt_ItemsList[initalCounter] != null)
            {
                receipt.ReceiptTotalItems[Items_in_Recipt_Counter] = Lunch_Recipt_ItemsList[initalCounter];
    ...
    private void Lunch_Menu_Selector()
    {        
        int ItemsSlected = 0;
        Scanner Lunch_Reader = new Scanner (System.in);  // Reading from System.in
        System.out.print("Please Enter The Digit that correspondence to what you want: ");
        int b = Lunch_Reader.nextInt();
        ...
        while (b != 0)
        {
            ...
            if (b== 9)
            {
                ...
                receipt.ReceiptTotalPrice += Lunch_TotalPrice;
                receipt.ReceiptTotalQuantity += Lunch_TotalQuantity;
            ...
            else if (b >= 1 && b <= 5)
            {
                ...
                System.out.println("You have selected " + Lunch_Item_Name[b-1]);
        
                // quantity of the item
                System.out.print("Please Enter The Quantity: ");
                int quantity = Lunch_Reader.nextInt();

                System.out.println("");
                System.out.println("You have selected " + quantity + " " + Lunch_Item_Name[b-1] + "(s)");
                
                // Append Price To Class Total Price
                selectedItem_Price = Lunch_Item_Price[b-1] * quantity;
                Lunch_TotalPrice += selectedItem_Price;
                Lunch_TotalQuantity += quantity;
               
               
                // Add Items to Receipt Format
                Lunch_Recipt_ItemsList[ItemsSlected] = Receipt_Format(Lunch_Item_Name[b-1], Lunch_Item_Price[b-1], quantity);
                ItemsSlected++;
